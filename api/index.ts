...
router.post("/ocr", async (req, res) => {
  try {
    ...
    if (!image) {
      return res.status(400).json({ error: "Imej diperlukan untuk OCR." });
    }

    const apiKey = process.env.GEMINI_API_KEY;
    if (!apiKey) {
      console.error("GEMINI_API_KEY missing in environment");
      return res.status(500).json({ error: "Konfigurasi AI tidak dijumpai." });
    }
    ...
    if (!jsonMatch) {
      throw new Error("Gagal menghuraikan maklumat dari AI.");
    }

    const ocrResult = JSON.parse(jsonMatch[0]);
    res.json({ success: true, result: ocrResult });

  } catch (err: any) {
    console.error("OCR API Error:", err);
    res.status(500).json({ error: err.message || "Gagal memproses OCR." });
  }
});
